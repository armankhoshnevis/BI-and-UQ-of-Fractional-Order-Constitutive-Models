...
from Inference_Funcs_FMM import (
    load_data,
    plot_diagnostic,
...
idata_dict = {}
for GnP in GnPs:
    idata_dict[GnP] = az.from_netcdf(f"../Results/MCMC_FMM_{HS}HSWF/EpEpp_FMM_{HS}HS_{GnP}.nc")
df_mean_inference = pd.read_csv(f"../Results/MCMC_FMM_{HS}HSWF/Mean_Inference_{HS}HS_FMM.csv")
idata_mean_dict = df_mean_inference.set_index('GnP').to_dict(orient='index')

df_std_inference = pd.read_csv(f"../Results/MCMC_FMM_{HS}HSWF/std_Inference_{HS}HS_FMM.csv")
idata_std_dict = df_std_inference.set_index('GnP').to_dict(orient='index')

df_map_estimate = pd.read_csv(f"../Results/MCMC_FMM_{HS}HSWF/Map_Estimate_{HS}HS_FMM.csv")
map_estimate_dict = df_map_estimate.set_index('GnP').to_dict(orient='index')

df_error_measures = pd.read_csv(f"../Results/MCMC_FMM_{HS}HSWF/Error_Measures_{HS}HS_FMM.csv")
...
for GnP_idx in range(len(GnPs)):
    plt_set = {
        "savefig_path": f"{file_path['file_path_save']}/Posterior_FMM_{HS}HS_{GnPs[GnP_idx]}.png"
    }
...
df_hdi_wide.to_csv(f"{file_path['file_path_save']}/Model_Param_HDI_FMM_{HS}HS.csv", index=False)
...
for GnP in idata_dict.keys():
    idata = idata_dict[GnP]
    plt_set = {
        "savefig_path_rank": f"{file_path['file_path_save']}/Rank_Plots_FMM_{HS}HS_{GnP}.png",
        "savefig_path_trace": f"{file_path['file_path_save']}/Trace_Plots_FMM_{HS}HS_{GnP}.png"
    }
...
plt_set = {
    "savefig_path": f"{file_path['file_path_save']}/Posterior_Predictive_FMM_{HS}HS_{GnPs[GnP_idx]}.png"
}
...
if HS == 20: 
    ...
    range2 = [2.5, 6.5]
plt_set = {
    "savefig_path": f"{file_path['file_path_save']}/Posterior_Predictive_Checks_FMM_{HS}HS_{GnPs[GnP_idx]}.png"
}
...
# --------------------------------------------------
plt_set = {
    "savefig_path": f"{file_path['file_path_save']}/Pairplot_FMM_{HS}HS_{GnPs[0]}.png"
}
